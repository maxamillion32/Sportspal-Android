...
public class TeamMembersManager {
    ...
    public ArrayList<Users> getNearUsers(String sportId,boolean shouldRefresh) {
        if (shouldRefresh)
            getUsers(sportId);
        return usersList;
    }

    private void getUsers(String sportId) {
        SPRestClient.get(ServiceApi.GET_USERS + "/" + ModelManager.getInstance().getAuthManager().getUserId()+"/"+sportId, null, new JsonHttpResponseHandler() {
            @Override
            public void onStart() {
            ...
            public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
                ...
                try {
                    ...
                    if (state == 200) {
                        ...
                        EventBus.getDefault().post("GetTeam True");
                    } else {
